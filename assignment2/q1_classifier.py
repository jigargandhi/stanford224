...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(dtype= tf.float32,shape= (self.config.batch_size, self.config.n_features), name='inputs_placeholder')
        self.labels_placeholder = tf.placeholder(dtype= tf.int32,shape= (self.config.batch_size, self.config.n_classes), name='labels_placeholder')
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.Variable(np.zeros((self.config.n_features, self.config.n_classes), dtype=np.float),dtype=tf.float32, name='W')
        b = tf.Variable(np.zeros((self.config.batch_size, self.config.n_classes), dtype=np.float), dtype = tf.float32, name='b')
        pred = softmax(tf.matmul(tf.to_float(self.input_placeholder), W)+b)
        ### END YOUR CODE
        return pred
